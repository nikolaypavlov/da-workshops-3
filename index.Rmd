...
```{r, echo=F, include=F}
t <- addmargins(table(email$spam, email$number, dnn=c("Spam", "Number")))
small_big <- t[3,2:3] / t[3, 4]
small_or_big <- sum(small_big)
...
```{r, echo=F, include=F}
small_and_spam <- t[2,2] / t[3, 4]
either <- (t[3,2] + t[2,4]) / t[3, 4] - small_and_spam
```
...
```{r, echo=F, include=F}
died_given_notinoc <- 844 / 5980
died_given_inoc <- 6 / 244
```
